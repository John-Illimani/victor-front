...
import { Plus, Trash2, MapPin, Users, GraduationCap } from 'lucide-react';
...
export const ActaConformacion3erAno = ({ isOpen, onClose, fichaData, setFichaData, estudianteSeleccionado }) => {
  ...
  return (
    <ModalBase isOpen={isOpen} onClose={onClose} titulo={config?.titulo || "ACTA DE CONFORMACIÓN Y COMPROMISO - 3ER AÑO"}>
      <div className="space-y-4 text-xs font-sans">
        <div className="bg-slate-50 p-5 rounded-3xl border border-slate-200 space-y-4">
          <h4 className="font-extrabold text-[#801B28] uppercase text-center text-sm border-b pb-2">
            ACTA DE CONFORMACIÓN Y COMPROMISO DEL EQUIPO COMUNITARIO DE INVESTIGACIÓN EDUCATIVA Y PRODUCCIÓN DE CONOCIMIENTOS - PEC
          </h4>
            ...
            <div className="space-y-2">
              {formData.integrantes.map((int, idx) => (
                <div key={idx} className="flex gap-2 items-center bg-slate-50 p-2 rounded-xl border border-slate-200">
                  ...
                  <input type="text" placeholder="NRO. CELULAR" value={int.celular} onChange={(e) => handleIntegranteChange(idx, 'celular', e.target.value)} className="w-32 border p-1.5 rounded-lg bg-white font-mono" />

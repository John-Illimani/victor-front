...
export const FichaA1_3erAno = ({ isOpen, onClose, fichaData, setFichaData, listaDocentes = [], estudianteSeleccionado }) => {
  const codigoFicha = "3_A1";
  const config = CONFIGURACION_FICHAS[codigoFicha];

  const criterios = [
    { key: 'nota_diseno', label: 'DISEÑO Y VALIDACIÓN DE TÉCNICAS E INSTRUMENTOS DE INVESTIGACIÓN' },
    { key: 'nota_aplicacion', label: 'APLICACIÓN DE TÉCNICAS E INSTRUMENTOS DE INVESTIGACIÓN' },
    { key: 'nota_analisis', label: 'ORDEN, ANÁLISIS, REFLEXIÓN E INTERPRETACIONAL DE LA INFORMACIÓN' }
  ];
          ...
          <div className="bg-amber-50/70 p-4 rounded-2xl border border-amber-200 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
              ...
              <div>
                ...
                <input type="text" value={formData.especialidad} onChange={(e) => handleCampoChange('especialidad', e.target.value)} className="w-full border border-amber-200 p-2 rounded-xl bg-white font-bold" />
              </div>
            </div>
          </div>

          {/* EVALUACIÓN MATRIZ A-1 */}
          <div className="bg-white p-4 rounded-2xl border border-slate-200 space-y-3">
            <span className="font-extrabold text-slate-800 uppercase text-[11px] block">
              MATRIZ DE VALORACIÓN FICHA A-1
            </span>

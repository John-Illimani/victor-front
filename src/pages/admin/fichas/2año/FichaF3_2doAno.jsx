...
export const FichaF3_2doAno = ({ isOpen, onClose, fichaData, setFichaData, listaDocentes = [], estudianteSeleccionado }) => {
  const codigoFicha = "2_F3";
  ...
  // Cargar datos de Ficha F-3
  useEffect(() => {
    const fetchFicha = async () => {
      if (isOpen && estudianteSeleccionado?.id) {
        setLoading(true);
        try {
          const res = await fichaF3_2doAnoService.getByEstudiante(estudianteSeleccionado.id);
          ...
          if (res.existe && res.datos) {
            ...
            criterios.forEach((c, idx) => {
              const valorBD = d[`f3_criterio_${idx + 1}`] ?? d[c.key];
              notasMapeadas[c.key] = valorBD !== undefined && valorBD !== null ? String(valorBD) : '';
              notasMapeadas[`f3_criterio_${idx + 1}`] = valorBD !== undefined && valorBD !== null ? String(valorBD) : '';
            });
          ...
          mostrarNotificacion("Error", error.message || "No se pudo consultar la Ficha F-3.", "error");
        } finally {
          setLoading(false);
        }
      }
  ...
  const handleNotaChange = (key, value, idx) => {
    ...
    const newForm = {
      ...formData,
      [key]: rawVal,
      [`f3_criterio_${idx + 1}`]: rawVal
    };

    const notas = criterios.map((c, i) => {
      const valStr = newForm[c.key] ?? newForm[`f3_criterio_${i + 1}`];
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fichaF3_2doAnoService.saveOrUpdate(estudianteSeleccionado.id, formData);
  ...
  const ejecutarEliminacion = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fichaF3_2doAnoService.delete(estudianteSeleccionado.id);
      const clearedData = {
        ...initialFormState,
        apellidos_nombres: `${estudianteSeleccionado.nombre || ''} ${estudianteSeleccionado.apellido || ''}`.trim(),
      ...
      mostrarNotificacion("Registro Eliminado", res.message || "Ficha F-3 eliminada con éxito.", "success");
    } catch (error) {
      setModalConfirmDelete(false);
  ...
  return (
    <>
            ...
            <div className="bg-slate-50 p-5 rounded-3xl border border-slate-200 space-y-4">
              ...
              <div className="flex flex-wrap justify-between items-center gap-2 border-b pb-2">
                <h4 className="font-extrabold text-[#801B28] uppercase text-sm">
                  {config?.titulo || "FICHA F-3: APLICACIÓN DE TÉCNICAS E INSTRUMENTOS"}
                </h4>
              ...
              <div className="bg-white p-4 rounded-2xl border border-slate-200 space-y-3">
                <span className="font-extrabold text-slate-800 uppercase text-[11px] block">CRITERIOS DE EVALUACIÓN F-3</span>
                  ...
                  {criterios.map((crit, idx) => (
                    <div key={crit.key} className="grid grid-cols-1 sm:grid-cols-4 gap-2 items-center bg-slate-50 p-2.5 rounded-xl border border-slate-200">
                      ...
                      <input
                        ...
                        step="0.1"
                        placeholder="0-100"
                        value={formatInputValue(formData[crit.key] ?? formData[`f3_criterio_${idx + 1}`] ?? '')}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Observaciones de la aplicación de técnicas e instrumentos..."
                  />
                </div>
              </div>
      ...
      <ConfirmModal
        ...
        mensaje="Esta acción eliminará la Ficha F-3 de la base de datos de manera permanente."
        tipo="danger"
        cargando={deleting}
      />
    </>

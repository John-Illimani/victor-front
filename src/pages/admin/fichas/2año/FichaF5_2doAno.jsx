...
export const FichaF5_2doAno = ({ isOpen, onClose, fichaData, setFichaData, listaDocentes = [], estudianteSeleccionado }) => {
  ...
  const initialFormState = {
    ...
    ano: '2026',
    f5_criterio_1: '', f5_obs_1: '',
    f5_criterio_2: '', f5_obs_2: '',
    f5_criterio_3: '', f5_obs_3: '',
    f5_criterio_4: '', f5_obs_4: '',
    f5_criterio_5: '', f5_obs_5: '',
    puntaje_final: '0',
  ...
  useEffect(() => {
    const fetchFicha = async () => {
      if (isOpen && estudianteSeleccionado?.id) {
        setLoading(true);
        try {
          const res = await fichaF5_2doAnoService.getByEstudiante(estudianteSeleccionado.id);
          const nombreCompleto = `${estudianteSeleccionado.nombre || ''} ${estudianteSeleccionado.apellido || ''}`.trim();

          if (res.existe && res.datos) {
            const d = res.datos;
            const prom = parseFloat(d.puntaje_final || 0);

            const todasLasClaves = etapas.flatMap(e => e.criterios.map(c => c.key));
            const notasMapeadas = {};
            todasLasClaves.forEach((k, idx) => {
              const valorBD = d[`f5_criterio_${idx + 1}`] ?? d[k];
              const obsBD = d[`f5_obs_${idx + 1}`] ?? d[`obs_${k}`] ?? '';

              notasMapeadas[k] = valorBD !== undefined && valorBD !== null ? String(valorBD) : '';
              notasMapeadas[`f5_criterio_${idx + 1}`] = valorBD !== undefined && valorBD !== null ? String(valorBD) : '';
              notasMapeadas[`f5_obs_${idx + 1}`] = obsBD;
            });
            ...
            setFormData(prev => ({
              ...
              puntaje_final: formatInputValue(prom),
          ...
          mostrarNotificacion("Error", error.message || "No se pudo consultar la Ficha F-5.", "error");
        } finally {
          setLoading(false);
        }
      }
  ...
  const handleEtapaChange = (key, val, idxGlobal) => {
    let rawVal = val;
    ...
    const nuevos = {
      ...formData,
      [key]: rawVal,
      [`f5_criterio_${idxGlobal + 1}`]: rawVal
    };

    const todasLasClaves = etapas.flatMap(e => e.criterios.map(c => c.key));
    const notas = todasLasClaves.map((k, i) => {
      const valStr = nuevos[k] ?? nuevos[`f5_criterio_${i + 1}`];
    ...
    nuevos.puntaje_final = formatInputValue(prom);
    nuevos.promedio_final = formatInputValue(prom);
    nuevos.promedio_numeral = formatInputValue(prom);
    nuevos.promedio_literal = convertirNumeroALiteral(prom);

    setFormData(nuevos);
    if (setFichaData) setFichaData(nuevos);
  };

  const handleObsCriterioChange = (idxGlobal, val) => {
    const updated = { ...formData, [`f5_obs_${idxGlobal + 1}`]: val };
    setFormData(updated);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fichaF5_2doAnoService.saveOrUpdate(estudianteSeleccionado.id, formData);
      mostrarNotificacion("¡Guardado Exitoso!", res.message || "La Ficha F-5 fue registrada correctamente.", "success");
    } catch (error) {
  ...
  const ejecutarEliminacion = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fichaF5_2doAnoService.delete(estudianteSeleccionado.id);
      const clearedData = {
        ...initialFormState,
        apellidos_nombres: `${estudianteSeleccionado.nombre || ''} ${estudianteSeleccionado.apellido || ''}`.trim()
      };
      ...
      mostrarNotificacion("Registro Eliminado", res.message || "Ficha F-5 eliminada con éxito.", "success");
    } catch (error) {
      setModalConfirmDelete(false);
  ...
  const footerButtons = (
    <div className="flex justify-end gap-2 w-full">
      ...
      >
        ...
        {saving ? "Guardando..." : "Guardar Ficha F-5"}
      </button>
    </div>
  );

  let globalCritIndex = 0;
  ...
  return (
    <>
            ...
            <div className="bg-slate-50 p-5 rounded-3xl border border-slate-200 space-y-4">
              ...
              <div className="flex flex-wrap justify-between items-center gap-2 border-b pb-2">
                <h4 className="font-extrabold text-[#801B28] uppercase text-sm">
                  {config?.titulo || "FICHA F-5: EVALUACIÓN POR ETAPAS PEC"}
                </h4>
              ...
              {/* CRITERIOS DE EVALUACIÓN Y OBSERVACIONES INDIVIDUALES */}
              <div className="bg-white p-4 rounded-2xl border border-slate-200 space-y-4">
                <span className="font-extrabold text-slate-800 uppercase text-[11px] block border-b pb-2">
                  CRITERIOS DE EVALUACIÓN Y OBSERVACIONES POR ETAPA (5 CRITERIOS)
                </span>
                  ...
                  <div key={idx} className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200 space-y-3">
                    ...
                    <div className="space-y-3">
                      {etapaObj.criterios.map((crit) => {
                        const currentIdx = globalCritIndex++;
                        const obsKey = `f5_obs_${currentIdx + 1}`;

                        return (
                          <div key={crit.key} className="bg-white p-3 rounded-xl border border-slate-200 space-y-2">
                            <div className="grid grid-cols-1 sm:grid-cols-4 gap-2 items-center">
                              <span className="sm:col-span-3 font-semibold text-slate-800 text-xs leading-snug">
                                {crit.label}
                              </span>
                              <input
                                type="number"
                                ...
                                max="100"
                                step="0.1"
                                value={formatInputValue(formData[crit.key] ?? formData[`f5_criterio_${currentIdx + 1}`] ?? '')}
                                onChange={(e) => handleEtapaChange(crit.key, e.target.value, currentIdx)}
                                className="w-full border border-slate-300 p-1.5 rounded-lg font-mono font-bold text-center text-slate-900 focus:border-[#801B28] outline-none"
                                placeholder="0-100"
                              />
                            </div>

                            {/* Observación debajo de cada criterio */}
                            <div>
                              <label className="block font-bold text-slate-500 text-[9px] uppercase mb-0.5">
                                Observación Criterio {currentIdx + 1}:
                              </label>
                              <input
                                type="text"
                                value={formData[obsKey] || ''}
                                onChange={(e) => handleObsCriterioChange(currentIdx, e.target.value)}
                                className="w-full border border-slate-200 p-1.5 rounded-lg font-medium text-slate-700 text-xs focus:border-[#801B28] outline-none"
                                placeholder={`Escriba la observación para el criterio ${currentIdx + 1}...`}
                              />
                            </div>
                          </div>
                        );
                ...
                <div className="bg-rose-50/60 p-4 rounded-2xl border border-rose-200 grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
                  <div>
                    ...
                    <div className="font-mono font-black text-2xl text-[#801B28]">
                      {formatInputValue(formData.puntaje_final) || '0'} / 100 PTS
                    </div>
                  </div>
                  <div>
                ...
                {/* OBSERVACIÓN GENERAL / SUGERENCIAS */}
                <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200">
                  <label className="block font-bold text-slate-700 text-[10px] uppercase mb-1">
                    Observaciones Generales y/o Sugerencias del Evaluador:
                  </label>
                  <textarea
                    rows="3"
      ...
      <ConfirmModal
        ...
        titulo="¿Eliminar Ficha F-5?"
        mensaje="Esta acción eliminará la Ficha F-5 de la base de datos de manera permanente."
        tipo="danger"
        cargando={deleting}
      />
    </>

...
export const FichaF5_1erAno = ({ isOpen, onClose, fichaData, setFichaData, listaDocentes = [], estudianteSeleccionado }) => {
  const codigoFicha = "1_F5";
  ...
  const formatInputValue = (val) => {
    ...
    // Si es entero -> devuelve entero sin decimales
    // Si tiene decimales -> devuelve el valor original con sus decimales
  ...
  // Carga de especialidades dinámicas desde la API
  useEffect(() => {
  ...
  // Cargar Ficha F-5 desde la API
  useEffect(() => {
    const fetchFicha = async () => {
      if (isOpen && estudianteSeleccionado?.id) {
        setLoading(true);
        try {
          const res = await fichaF5Service.getByEstudiante(estudianteSeleccionado.id);
  ...
  const handleNotaChange = (key, value) => {
    ...
    // Clampear nota estrictamente entre 0 y 100
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fichaF5Service.saveOrUpdate(estudianteSeleccionado.id, formData);
  ...
  const ejecutarEliminacion = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fichaF5Service.delete(estudianteSeleccionado.id);
      const clearedData = {
        ...initialFormState,
        apellidos_nombres: `${estudianteSeleccionado.nombre || ''} ${estudianteSeleccionado.apellido || ''}`.trim(),
        especialidad: estudianteSeleccionado.especialidad || initialFormState.especialidad
  ...
  return (
    <>
            ...
            <div className="bg-slate-50 p-5 rounded-3xl border border-slate-200 space-y-4">
              ...
              <div className="flex flex-wrap justify-between items-center gap-2 border-b pb-2">
                <h4 className="font-extrabold text-[#801B28] uppercase text-sm">
                  {config?.titulo || "FICHA F-5: EVALUACIÓN DE PRODUCCIÓN Y SISTEMATIZACIÓN"}
                </h4>
              ...
              {/* CRITERIOS DE PRODUCCIÓN OFICIALES */}
              <div className="bg-white p-4 rounded-2xl border border-slate-200 space-y-3">
                <span className="font-extrabold text-slate-800 uppercase text-[11px] block">EVALUACIÓN DE PRODUCCIÓN Y SISTEMATIZACIÓN</span>
                  ...
                  {criterios.map(crit => (
                    <div key={crit.key} className="grid grid-cols-1 sm:grid-cols-4 gap-2 items-center bg-slate-50 p-2.5 rounded-xl border">
                      ...
                      <input
                        ...
                        max={crit.max || 100}
                        placeholder="0-100"
                        /* VALIDACIÓN DE ENTERO DIRECTAMENTE EN EL VALUE DE LOS INPUTS */
                ...
                {config?.tieneObservaciones && (
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Observaciones de la producción de conocimientos..."
                    />
                  </div>
                )}
              </div>
      ...
      <ConfirmModal
        ...
        mensaje="Esta acción eliminará el registro de la Ficha F-5 de la base de datos de manera permanente."
        tipo="danger"
        cargando={deleting}
      />
    </>

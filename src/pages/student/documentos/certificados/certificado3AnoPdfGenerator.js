...
// FILAS DEL CENTRALIZADOR PARA EL 3ER AÑO DE FORMACIÓN
const FILAS_CENTRALIZADOR_3ER_ANO = [
  {
    etapaKey: "Etapa preparatoria",
    etapaRows: 1,
    actividad: "Técnicas e instrumentos de investigación",
    ficha: "A-1",
    fichaRows: 1,
    notaRows: 1,
    fieldKey: "nota_a1"
  ...
  {
    etapaKey: "Etapa de ejecución",
    etapaRows: 4,
    actividad: "Apoyo – seguimiento del Docente Acompañante de la ESFM/UA",
    ficha: "B-1",
    fichaRows: 1,
    notaRows: 1,
    fieldKey: "nota_b1"
  ...
  {
    etapaKey: null,
    actividad: "Asistencia práctica educativa comunitaria.",
    ficha: "B-2",
    fichaRows: 1,
    notaRows: 1,
    fieldKey: "nota_b2"
  ...
  {
    etapaKey: null,
    actividad: "Apoyo - Seguimiento del docente guía de la UE/ CEA/CEE, en la concreción curricular.",
    ficha: "B-3",
    fichaRows: 1,
    notaRows: 1,
    fieldKey: "nota_b3"
  ...
  {
    etapaKey: null,
    actividad: "Seguimiento y apoyo del docente tutor/acompañante",
    ficha: "B-4",
    fichaRows: 1,
    notaRows: 1,
    fieldKey: "nota_b4"
  ...
  {
    etapaKey: "Etapa de producción",
    etapaRows: 1,
    actividad: "Presentación del informe diagnóstico socioparticipativo",
    ficha: "B-5",
    fichaRows: 1,
    notaRows: 1,
    fieldKey: "nota_b5"
...
export const imprimirCertificado3erAno = async (estudianteId, blockchainData = {}) => {
  try {
    const responseCentralizador = await centralizador3erAnoService.getByEstudiante(estudianteId);

    if (!responseCentralizador || !responseCentralizador.datos) {
      return { success: false, message: "No existen datos registrados para el Centralizador de 3er Año." };
    }
    ...
    const urlPlantilla = "/pdf/3año/plantillaCertificado.pdf";
    let pdfDoc;
    ...
    // 4. TABLA CENTRALIZADORA DE EVALUACIÓN 3ER AÑO
    ...
    const seccionText = "CENTRALIZADOR DE LA EVALUACIÓN - 3ER AÑO";
    ...
    FILAS_CENTRALIZADOR_3ER_ANO.forEach((row, rowIndex) => {
      const rowTop = currentY;

      const actLines = wrapTextToLines(row.actividad, fontBold, colWidths[1] - 10, 7.5);
      const rowH = Math.max(actLines.length * 8.5 + 8, 20);
      ...
      if (etapaInfoCurrent) {
        ...
        if (etapaInfoCurrent.rowsCount === 0) {
          ...
          const etapaLines = wrapTextToLines(etapaInfoCurrent.text, fontBold, colWidths[0] - 8, 7);
          const lineHeight = 8.5;
          const totalTextH = etapaLines.length * lineHeight;
    ...
    const pStr = formatNotaEntero(d.promedio_numeral || d.promedio_final);
    if (pStr) {
    ...
    const litRowH = 18;
    const litText = `Literal: ${numeroALiteralEntero(d.promedio_numeral || d.promedio_final)}`;

    page.drawText(litText, {
      x: colX[0] + 8,
      y: literalTop - 12,
      size: 8.5,
      font: fontBold,
    ...
    vLine(page, colX[3], tableTop - tableHeaderH, promFinalTop - promFinalH);
    ...
    console.error("Error al generar PDF del Certificado de 3er Año:", error);

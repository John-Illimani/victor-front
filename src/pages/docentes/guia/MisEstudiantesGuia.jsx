...
export const MisEstudiantesGuia = () => {
  ...
  // CARGAR ESTUDIANTES ASIGNADOS AL AULA DEL DOCENTE GUÍA
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        if (!docenteId) {
          setErrorMessage("No se encontró el identificador del docente guía autenticado.");
          setLoading(false);
          return;
        }
        ...
        setErrorMessage("Error de conexión al cargar la nómina de estudiantes asignados.");
      } finally {
        setLoading(false);
      }
    };
  ...
  // FILTRADO POR BÚSQUEDA Y AÑO DE FORMACIÓN
  ...
  // MALLA DE FICHAS SEGÚN EL AÑO DE FORMACIÓN
  ...
  // CARGAR LA FICHA Y MOSTRAR SU CONTENIDO
  ...
  // OBTIENE EL COMPONENTE ASOCIADO
        ...
        <div className="relative z-10 flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-2">
            ...
            <p className="text-xs sm:text-sm text-slate-300 max-w-xl leading-relaxed">
              Consulta de registros, actas y fichas de seguimiento IEPC-PEC de los estudiantes practicantes asignados a tu aula.
            </p>
          </div>
        </div>
      </div>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan="6" className="py-8 text-center text-slate-400 font-medium">
                    No se encontraron estudiantes practicantes asignados en tu aula.
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="relative w-full max-w-2xl rounded-3xl bg-white p-5 sm:p-6 shadow-2xl border border-slate-100 max-h-[90vh] overflow-y-auto">
            ...
            <div className="border-b border-slate-100 pb-3 mb-4">
              <span className="text-[10px] font-extrabold uppercase text-[#8C731A]">
                CONSULTA DE EXPEDIENTE EN AULA
              </span>
        ...
        <React.Fragment>
          ...
          <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[9999] bg-slate-900/90 backdrop-blur-md px-6 py-2.5 rounded-2xl shadow-2xl border border-white/20 flex items-center gap-4">
            <span className="text-xs font-extrabold text-white hidden sm:inline">
              VISTA DE SOLO LECTURA — DOCENTE GUÍA
            </span>
            <button
              onClick={() => setActiveFicha(null)}

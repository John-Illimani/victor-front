  ...
  useEffect(() => {
    const initData = async () => {
      ...
      try {
        ...
        // Extraer únicamente las FICHAS en TRUE desde BD
        ...
        // Cargar lista de estudiantes asignados al aula del Docente Guía
        ...
        setErrorMessage("Error de conexión al cargar las fichas asignadas.");
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-extrabold uppercase tracking-widest text-[#F3EFCF] backdrop-blur-md border border-white/15">
              <ClipboardCheck size={14} className="text-[#8C731A]" /> Control y Valoración en Aula (Docente Guía)
            </div>
            <h1 className="text-2xl sm:text-4xl font-black tracking-tight text-white flex items-center gap-3">
              Fichas Asignadas Habilitadas
              <Sparkles size={26} className="text-[#8C731A] animate-pulse shrink-0" />
            </h1>
            <p className="text-xs sm:text-sm text-slate-300 max-w-xl leading-relaxed">
              Formularios de asistencia y evaluación pedagógica asignados para la práctica del estudiante en el aula.
            </p>
          </div>
        </div>
      </div>

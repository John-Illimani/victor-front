...
export const fichaB53erAnoService = {
  getByEstudiante: async (estudiante_id) => {
    try {
      const response = await api.get(`/3er-ano/ficha-b5/${estudiante_id}`);
  ...
  saveOrUpdate: async (estudiante_id, datos) => {
    try {
      const response = await api.post("/3er-ano/ficha-b5/guardar", { estudiante_id, datos });
      return response.data;
    } catch (error) {
  ...
  delete: async (estudiante_id) => {
    try {
      const response = await api.delete(`/3er-ano/ficha-b5/${estudiante_id}`);
      return response.data;
    } catch (error) {

...
export const fichaB44toAnoService = {
  getByEstudiante: async (estudiante_id) => {
    try {
      const response = await api.get(`/4to-ano/ficha-b4/${estudiante_id}`);
  ...
  saveOrUpdate: async (estudiante_id, datos) => {
    try {
      const response = await api.post("/4to-ano/ficha-b4/guardar", { estudiante_id, datos });
      return response.data;
    } catch (error) {
  ...
  delete: async (estudiante_id) => {
    try {
      const response = await api.delete(`/4to-ano/ficha-b4/${estudiante_id}`);
      return response.data;
    } catch (error) {
